<template>
    <my-base ref="base">
        <div class="mask">
            ...
            <div class="line data">
                ...
                <div class="table">
                    ...
                    <Table border :columns="table.field" :data="table.data" @on-selection-change="selectedEvent">
                        ...
                        <template v-slot:s_ico="{row,index}"><img :src="row.__s_ico__ ? row.__s_ico__ : $store.state.context.res.notFound" class="image" height="40" @click.stop="link(row.__s_ico__)"></template>
                        <template v-slot:b_ico="{row,index}"><img :src="row.__b_ico__ ? row.__b_ico__ : $store.state.context.res.notFound" class="image" height="40" @click.stop="link(row.__b_ico__)"></template>
                        ...
                        <template v-slot:is_view="{row,index}"><my-switch v-model="row.is_view" :loading="val.pending['is_view' + row.id]" :extra="{id: row.id , field: 'is_view'}" @on-change="updateBoolValEvent" /></template>
                        <template v-slot:enable="{row,index}"><my-switch v-model="row.enable" :loading="val.pending['enable' + row.id]" :extra="{id: row.id , field: 'enable'}" @on-change="updateBoolValEvent" /></template>
            ...
            <my-form :title="drawerTitle" :form="form" :mode="val.mode" v-model="val.drawer" :data="table.data" @on-success="getData"></my-form>
        </div>
    </my-base>
</template>
